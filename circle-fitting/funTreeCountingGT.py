...
def funTreeCountingGT(I):
    ...
    for i in range(1, int(L.max()) + 1):
        ...
        apoX, apoY, eosX, eosY = int(s.bbox[0]), int(s.bbox[1]), int(s.bbox[2]), int(s.bbox[3])
    ...
    if RUNRESIZE == 1:
        IClustTotal = cv2.resize(IClustTotal, (I0.shape[1], I0.shape[0]), interpolation=cv2.INTER_NEAREST)
        
        BW = cv2.resize(BW, (I0.shape[1], I0.shape[0]), interpolation=cv2.INTER_NEAREST)
    
    stats = regionprops_table(IClustTotal.astype(int), properties=['centroid', 'area', 'equivalent_diameter'])
